...
```text
    1、.*
      .代表匹配初换行符\n之外的任何字符，*标示零次或者多次；所以 .* 在一起就表示任意字符出现零次或者多次；
      没有？表示贪婪模式。比如 a.*b，匹配最长的以a开始，以b结束的字符串，如果将其应用于aabab的话，他会匹配aabab。
    2、 .*?
      ?跟在*或者+后面，表示懒惰模式，或非贪婪模式，匹配尽可能少的字符；比如 a.*?b 匹配最短的，以a开始，以b结束的字符串，如果将其应用于aabab的话，他会匹配aab和ab两个
    3、.+?
      表示匹配至少一个尽可能少的字符，如果将其应用于ababccaab的话，会匹配到abab和aab。
```
